...
void setGLSLshaders(char* vertexShaderFileName, char* fragmentShaderFileName)
{
	...
	glShaderSource(vertexShader, 1, (const GLchar**)&VSsourceCodes, NULL);
	glCompileShader(vertexShader);
	print_shader_info_log(vertexShader);

	fprintf(stderr, "Fragment ShaderSources: [\n%s\n]\n", FSsourceCodes);
	glShaderSource(fragmentShader, 1, (const GLchar**)&FSsourceCodes, NULL);
	...
	//Designate texture unit 0 in the fragment shader
	/*int texLoc = glGetUniformLocation(myProgObjID, "myTex1");
	glUniform1i(texLoc, 0);
	//Designate texture unit 1 in the fragment shader
	int texLoc2 = glGetUniformLocation(myProgObjID, "myTex2");
